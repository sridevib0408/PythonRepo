    ...
    def processEmailRequest(self,datefrm,dateto):
        ...
        for mailItem in self.getMessages():
            ...
            if mailRecvTime > from_date and mailRecvTime < to_date:
                lines = mailItem.body.split("\n")
                for msg in lines:
                    line = msg.lower()
                    data = re.findall(r'mts(.*)',line)
                    if data:
                        data = 'mts'+ ''.join(data)
                        data1 = pandas.DataFrame({"Data":[data],"Date": [mailRecvTime]})
                        df = df.append(data1)

        if not(df.empty):
            try:
                book = load_workbook('C:\projects\log_panda.xlsx')
                writer = pandas.ExcelWriter('C:\projects\log_panda.xlsx', engine='openpyxl')
                writer.book = book
